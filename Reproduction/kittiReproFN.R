...
x<- (cbind(as.numeric(dfA2$C_age),as.numeric( dfA2$F_Age)))
...
dfA2$F_Age<- as.numeric(dfA2$F_Age)

fitA<- brm(formula = A~C_age+ F_Age,
           data = dfA2,
...
fitS<- brm(formula = S~C_age+ F_Age,
           data = dfS,
           family = bernoulli(link = "logit"),
           iter=20000, warmup = 10000)
...
fitN<- brm(formula = N~C_age+ F_Age,
           data = dfN,
           family = bernoulli(link = "logit"),
           iter=20000, warmup = 10000)
...
save(fitA, fitS, fitN, file = "fitreproFN")
...
class(dfA3$C_age)
dfA3$F_Age<- as.numeric(dfA3$F_Age)
dfA3$C_age<- as.numeric(dfA3$C_age)



fitPA_A<- brm(formula = A ~ F_Age,
              data = dfA3,
              family = bernoulli(link = "logit"),
              iter=20000, warmup = 10000)

WAIC_PA_A<- waic(fitPA_A)


fitCS_A<- brm(formula = A ~ F_Age*CS,
           data = dfA3,
           family = bernoulli(link = "logit"),
           iter=20000, warmup = 10000)
...
fitTotA<- brm(formula = A ~ C_age+ F_Age*CS,
           data = dfA3,
           family = bernoulli(link = "logit"),
           iter=20000, warmup = 10000)
...
save(fitPA_A, fitCS_A, fitTotA,fitH0,fitA,  WAIC_CS_A, WAIC_CA_A, WAIC_TOT_A,WAIC_H0, WAIC_PA_A, file = "fitCS_1_FN.RDATA")
...
fitCS_S<- brm(formula = S ~ F_Age*CS,
              data = df3S,
              family = bernoulli(link = "logit"),
              iter=20000, warmup = 10000)
...
fitTotS<- brm(formula = S ~ C_age+ F_Age*CS,
              data = df3S,
              family = bernoulli(link = "logit"),
              iter=20000, warmup = 10000)
...
fitCS_N<- brm(formula = N ~ F_Age*CS,
              data = df3N,
              family = bernoulli(link = "logit"),
              iter=20000, warmup = 10000)
...
fitTotN<- brm(formula = N ~ C_age+ F_Age*CS,
              data = df3N,
              family = bernoulli(link = "logit"),
              iter=20000, warmup = 10000)
...
save(fitCS_S,fitS, fitTotS,fitH0S, WAIC_CS_S, WAIC_CA_S, WAIC_TOT_S,WAIC_H0_S , file = "fitCS_S_FN.RDATA")

save(fitCS_N,fitN, fitTotN,fitH0N, WAIC_CS_N, WAIC_CA_N, WAIC_TOT_N,WAIC_H0_N , file = "fitCS_N_FN.RDATA")
