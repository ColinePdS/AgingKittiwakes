...
colnames(DATA)[3]<- 'FatherAge'
colnames(DATA)[5]<- 'LRS'
colnames(DATA)[9]<- 'SexChick'
...
DATA2<- DATA
DATA2[DATA2$MotherAge >= 15, 1] <- 15 #subset(DATA, DATA$MotherAge<=20)
nrow(DATA2)



colnames(DATA2)[5]<- 'lrs'
...
DATA4<- DATA3
DATA4[DATA4$MotherAge >= 15,1 ] <- 15 #subset(DATA, DATA$MotherAge<=20)
nrow(DATA4)


#####################models----
data = list(N = nrow(DATA4), x = DATA4$MotherAge, y = DATA4$lrs, kp = 12, Sex = SexB)#, betakp_lower = 1, betakp_upper = 20)
...
data = list(N = nrow(DATA4), x = DATA4$MotherAge, y = DATA4$lrs)
...
data = list(N = nrow(DATA4), x = DATA4$MotherAge, y = DATA4$lrs, Sex = SexB)#, betakp_lower = 1, betakp_upper = 20)
...
##################averaging----
save(fit_BS12CS, fit_ZIP, fit_ZIPCS, fit_0, file = "fitLRSMNmu.RDATA")
...
################
save (wts1, file = 'wts1MN.RDATA')
load (file = 'wts1MN.RDATA')

load('fitLRSMNmu.RDATA')
...
(ggplot(data = tv1, mapping = aes(x = V1, y = tv1$V2 ))
  + geom_line(y = tv1$V3, color = 'red')
  + geom_line(y =  tv1$V4, color = 'blue')
  
)

plot(x = tv1$V1, y = tv1$V2,
     abline(a = alphaF, b = betaF),
     
)
abline(a = alphaM, b = betaM)

plot(x = tv1$V1, y = tv1$V2, col = "white",
     main = 'LRS function of  the Mother Age',
     xlab = 'Mother Age', ylab = 'LRS',
...
save(alphaF, alphaM, betaF, betaM, file = "muLRSMN.RDATA")
...
load(file = "muLRSFN.RDATA")

plot(x = tv1$V1, y = tv1$V2, col = "white",
     main = 'LRS function of  the Father Age',
